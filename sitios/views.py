...
from sitios.forms import NewUserForm, NewPassForm, BuscaSitiosForm, VoteForm,\
    ...
    RecomiendaSitiosForm, PublicidadForm, ContactForm, TakeoutForm
...
def buscar_sitios_cercanos_coordenadas(request, ciudad, lat, lng):
    ...
    while len(sitios_ids) == 0 and radio < 15:
        ...
        if query:
            cursor.execute("SELECT id, ( 6371 * acos( cos( radians(%s) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians(%s) ) + sin( radians(%s) ) * sin( radians( lat ) ) ) ) AS distance FROM sitios_sitio WHERE slug LIKE %s HAVING distance < %s ORDER BY distance LIMIT 0 , 20;", (lat, lng, lat, '%'+query+'%', radio))
        else:
            cursor.execute("SELECT id, ( 6371 * acos( cos( radians(%s) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians(%s) ) + sin( radians(%s) ) * sin( radians( lat ) ) ) ) AS distance FROM sitios_sitio HAVING distance < %s ORDER BY distance LIMIT 0 , 20;", (lat, lng, lat, radio))
...
def index(request, ciudad, mobile=False):	
    ...
    #sitios_populares = Sitio.objects.filter(ciudad=cod_ciudad).order_by('-num_votos', '-rank')[:5]
    sitios_populares = Sitio.objects.filter(ciudad=cod_ciudad).extra(select={
                        'hit_count': 'SELECT hits FROM hitcount_hit_count as t \
                        WHERE t.content_type_id = 14 \
                        AND t.object_pk = sitios_sitio.id',
...
def takeout(request):
    ...
    all_fotos = Foto.objects.filter(user=request.user, flickr=False)
    all_comentarios = Comentario.objects.filter(user=request.user)
    all_sitios_enviados = Sitio.objects.filter(user=request.user)
    datos_usuario = DatosUsuario.objects.get(user=request.user)
    all_sitios_favoritos = datos_usuario.favoritos.all()
    
    c = {'title': _(u'Exporta tus datos'),
        'user': request.user,
        'ciudad': ciudad,
        'number_fotos': all_fotos.count(),
        'number_comentarios': all_comentarios.count(),
        'number_sitios_enviados': all_sitios_enviados.count(),
        'number_sitios_favoritos': all_sitios_favoritos.count()
    }
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            sitios_enviados = form.cleaned_data['sitios_enviados']
            sitios_favoritos = form.cleaned_data['sitios_favoritos']
            
            zip_filename = "andaluciapeople-data.zip"
            s = StringIO.StringIO()
            ...
            if fotos:
                ...
                for foto in all_fotos:
                    paths_fotos.append(settings.MEDIA_ROOT + "/" + foto.get_path())
                
                zip_subdir = "photos"
                
                for fpath in paths_fotos:
            ...
            if comentarios:
                ...
                for comentario in all_comentarios:
                    res.append({"sitio": comentario.sitio.nombre,
                                "mensaje": comentario.mensaje,
                                "fecha": comentario.fecha.isoformat()})
                comentarios_json = simplejson.dumps(res)
                s_comentarios = StringIO.StringIO()
                s_comentarios.write(comentarios_json)
                zf.writestr("comentarios.json", s_comentarios.getvalue())
            
            if sitios_enviados:
                res = []
                for sitio in all_sitios_enviados:
                    res.append({"nombre": sitio.nombre,
                                "slug": sitio.slug,
                                "direccion": sitio.direccion,
                                "zona": sitio.zona,
                                "ciudad": sitio.ciudad,
                                "tipo": [x.tipo for x in sitio.tipo.all()],
                                "lat": sitio.lat,
                                "lng": sitio.lng,
                                "telefono": sitio.telefono,
                                "web": sitio.web,
                                "fecha": sitio.fecha.isoformat()})
                sitios_enviados_json = simplejson.dumps(res)
                s_sitios_enviados = StringIO.StringIO()
                s_sitios_enviados.write(sitios_enviados_json)
                zf.writestr("sitios_enviados.json", s_sitios_enviados.getvalue())
                
            if sitios_favoritos:
                res = []
                for sitio in all_sitios_favoritos:
                    res.append({"nombre": sitio.nombre,
                                "slug": sitio.slug,
                                "direccion": sitio.direccion,
                                "zona": sitio.zona,
                                ...
                                "web": sitio.web,
                                "fecha": sitio.fecha.isoformat()})
                sitios_favoritos_json = simplejson.dumps(res)
                s_sitios_favoritos = StringIO.StringIO()
                s_sitios_favoritos.write(sitios_favoritos_json)
                zf.writestr("sitios_favoritos.json", s_sitios_favoritos.getvalue())
    ...
    t = get_template('base_takeout.html')
    html = t.render(RequestContext(request, c))
    return HttpResponse(html)
